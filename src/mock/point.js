...
const generateOffers = () => {
  ...
  let offers = [];
  for (let i = 0, max = getRandomInteger(0, OffersList.length - 1); i < max; i++) {
    offers.push(OffersList[getRandomInteger(0, OffersList.length - 1)]);
  }
...
const generateDestination = () => {
  ...
  for (let i = 1, max = getRandomInteger(1, 5); i <= max; i++) {
    description += description ? ` ` : `` + TextList[getRandomInteger(0, TextList.length - 1)];
  }
...
const generatePoint = () => {
  ...
  return {
    ...
    time: {
      start: new Date(start), // Mon Aug 10 2020 00:00:00 GMT+0300
      end: new Date(getRandomInteger(start, end.getTime())) // плюс 2 суток
    }
  };
};
